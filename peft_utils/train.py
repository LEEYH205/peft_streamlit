...
try:
    from peft import (
        LoraConfig, IA3Config, PrefixTuningConfig, PromptTuningConfig,
...
def _trainer(base, tok, train_ds, eval_ds, out_dir, lr=5e-4, epochs=1, bs=2, grad_accum=4, fp16=True):
    ...
    return Trainer(model=base, args=args, train_dataset=train_ds, eval_dataset=eval_ds, data_collator=collator)

def build_adapter(base, method="lora", target_modules=None, r=8, alpha=16, dropout=0.05, num_virtual_tokens=16):
        ...
        if method == "lora":
            ...
            try:
                cfg = PrefixTuningConfig(
                    ...
                    encoder_hidden_size=base.config.hidden_size if hasattr(base.config, 'hidden_size') else 768
                )
            ...
            try:
                cfg = PromptTuningConfig(task_type=TaskType.CAUSAL_LM, num_virtual_tokens=num_virtual_tokens)
            except Exception as e:
...
def train_once(base, tok, train_ds, eval_ds, out_dir="outputs/demo", **kwargs):
    ...
    adapter_kwargs = {}
    if method == "lora":
        adapter_kwargs = {k: v for k, v in kwargs.items() if k in ["r", "alpha", "dropout", "target_modules"]}
    elif method == "ia3":
        adapter_kwargs = {k: v for k, v in kwargs.items() if k in ["target_modules"]}
    elif method in ["prefix", "prompt"]:
        adapter_kwargs = {k: v for k, v in kwargs.items() if k in ["num_virtual_tokens"]}
    ...
    try:
        tr = _trainer(model, tok, tokenized_train_ds, tokenized_eval_ds, out_dir, lr=kwargs.get("lr", 5e-4), epochs=kwargs.get("epochs", 1))
        tr.train()
        metrics = tr.evaluate()
        ...
        if isinstance(metrics, dict) and "eval_loss" in metrics:
            ...
            if hasattr(eval_loss, '__len__') and len(eval_loss) > 0:
                eval_loss = eval_loss[0] if hasattr(eval_loss, '__getitem__') else float(eval_loss)
            else:
