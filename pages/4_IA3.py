...
st.title(
    "⚖️ IA³ — 스케일링 벡터 학습 (Infused Adapter by Inhibiting and Amplifying Inner Activations)"
)
...
with st.sidebar:
    ...
    target_modules = st.text_input(
        "target_modules (쉼표 구분)", value="c_attn,c_proj", help="IA³를 적용할 모듈들"
    )
...
with col2:
    ...
    ax.fill_between(
        epochs, scaling_values - 0.1, scaling_values + 0.1, alpha=0.3, color="green"
    )
...
with col2:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    bars = ax.bar(
        methods, speed_data, color=["red", "orange", "yellow", "green"], alpha=0.7
    )
