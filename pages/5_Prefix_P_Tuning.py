...
st.set_page_config(
    page_title="Prefix & Prompt Tuning - 가상 토큰 학습", page_icon="📌", layout="wide"
)
...
with col2:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    bars = ax.bar(
        methods, param_counts, color=["red", "orange", "yellow", "green"], alpha=0.7
    )
...
if st.button("🚀 가상 토큰 데모 학습 실행", type="primary"):
    with st.spinner(f"{method.upper()} 모델 준비 중..."):
        try:
            ...
            st.info(
                "💡 현재 환경에서 Prefix/Prompt Tuning이 지원되지 않을 수 있습니다. LoRA나 IA³를 대신 사용해보세요."
            )
