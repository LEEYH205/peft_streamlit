...
with col1:
    ...
    # 일반 LoRA 구조
    ax.text(0.5, 0.9, '원본 가중치 (32-bit)', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue"))
    ax.arrow(0.5, 0.8, 0, -0.2, head_width=0.05, head_length=0.05, fc='red', ec='red')
    ax.text(0.5, 0.6, 'LoRA 어댑터', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightcoral"))
    ax.arrow(0.5, 0.5, 0, -0.2, head_width=0.05, head_length=0.05, fc='red', ec='red')
    ax.text(0.5, 0.3, '메모리: 100%', ha='center', va='center', fontsize=10,
...
with col2:
    ...
    ax.text(0.5, 0.6, '4-bit 양자화', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightyellow"))
    ax.arrow(0.5, 0.5, 0, -0.2, head_width=0.05, head_length=0.05, fc='orange', ec='orange')
    ax.text(0.5, 0.3, 'LoRA 어댑터', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightcoral"))
    ax.arrow(0.5, 0.2, 0, -0.2, head_width=0.05, head_length=0.05, fc='green', ec='green')
    ax.text(0.5, 0.0, '메모리: 12.5%', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgreen"))
...
with col1:
    ...
    # 양자화 수준별 메모리 사용량
    quantization_levels = ['32-bit (FP32)', '16-bit (FP16)', '8-bit (INT8)', '4-bit (INT4)']
    memory_usage = [100, 50, 25, 12.5]
    
    fig = create_comparison_chart(quantization_levels, memory_usage, 
                                 '양자화 수준별 메모리 사용량', '메모리 사용량 (%)')
    st.pyplot(fig)
...
with col2:
    ...
    for i, (mem, perf) in enumerate(zip(x, y)):
        ax.annotate(f'{quantization_levels[i].split()[0]}', 
                   (mem, perf), textcoords="offset points", 
    ...
    with st.spinner("QLoRA 모델 준비 중..."):
        ...
        try:
            metrics = train_once(base, tok, train_ds, eval_ds, method="lora",
                               r=r, alpha=alpha, dropout=dropout, target_modules=tm, epochs=epochs, lr=lr, four_bit=quant_ok)
