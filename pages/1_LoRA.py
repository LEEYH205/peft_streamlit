...
with st.sidebar:
    ...
    r = st.slider("LoRA rank (r)", 1, 64, 8, step=1, help="저랭크 행렬의 차원 (낮을수록 빠르지만 성능이 떨어질 수 있음)")
    alpha = st.slider("lora_alpha", 1, 128, 16, step=1, help="LoRA 스케일링 팩터 (보통 r의 2배로 설정)")
    dropout = st.slider("lora_dropout", 0.0, 0.5, 0.05, step=0.01, help="드롭아웃 비율 (과적합 방지)")
...
with col1:
    ...
    ax.text(0.5, 0.5, '전체 파라미터 학습', ha='center', va='center', fontsize=12,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightcoral"))
    ax.arrow(0.5, 0.4, 0, -0.2, head_width=0.05, head_length=0.05, fc='red', ec='red')
    ax.text(0.5, 0.2, '미세조정된 가중치', ha='center', va='center', fontsize=12,
...
with col2:
    ...
    # LoRA 구조
    ax.text(0.5, 0.9, '원본 가중치 (고정)', ha='center', va='center', fontsize=12,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightblue"))
    
    # A, B 행렬
    ax.text(0.3, 0.7, 'A 행렬 (r×d)', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.2", facecolor="lightyellow"))
    ax.text(0.7, 0.7, 'B 행렬 (d×r)', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.2", facecolor="lightyellow"))
    ...
    ax.text(0.3, 0.4, 'A 학습', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.2", facecolor="lightgreen"))
    ax.text(0.7, 0.4, 'B 학습', ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.2", facecolor="lightgreen"))
    
    ax.arrow(0.5, 0.3, 0, -0.2, head_width=0.05, head_length=0.05, fc='green', ec='green')
    ax.text(0.5, 0.1, 'LoRA 가중치', ha='center', va='center', fontsize=12,
            bbox=dict(boxstyle="round,pad=0.3", facecolor="lightgreen"))
...
with col1:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    bars = ax.bar(methods, param_counts, color=['red', 'lightgreen', 'green', 'darkgreen'], alpha=0.7)
...
with col2:
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    bars = ax.bar(methods, speed_data, color=['red', 'lightgreen', 'green', 'darkgreen'], alpha=0.7)
